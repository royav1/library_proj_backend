...
app.config['PROFILE_PICS_FOLDER'] = os.path.join(base_upload_folder, 'prof_pics')
app.config['BOOK_PICS_FOLDER'] = os.path.join(base_upload_folder, 'books_pics')
...
class User(db.Model):
    ...
    profile_pic = db.Column(db.String(200), nullable=True)  # New field for profile picture
...
class Book(db.Model):
    ...
    book_pic = db.Column(db.String(200), nullable=True)  # New field for book picture
...
def user_to_dict(user):
    return {
        ...
        'email': user.email,
        'active': user.active,
        'profile_pic': user.profile_pic  # Include this field to provide the path to the image
    }
...
def upload_file(upload_type):
    ...
    if file:
        ...
        elif upload_type == 'book':
            save_path = os.path.join(app.config['BOOK_PICS_FOLDER'], filename)
        else:
            return jsonify({'message': 'Invalid upload type'}), 400
        file.save(save_path)
        return jsonify({'message': 'File uploaded successfully', 'filename': filename, 'path': save_path}), 201

@app.route('/uploads/<upload_type>/<filename>')
def uploaded_file(upload_type, filename):
    if upload_type == 'profile':
        directory = app.config['PROFILE_PICS_FOLDER']
    elif upload_type == 'book':
        directory = app.config['BOOK_PICS_FOLDER']
    else:
        return jsonify({'message': 'Invalid upload type'}), 400
...
# Define a route to manage user profile, including image uploads
...
def manage_profile():
    ...
    if current_role not in ['customer', 'admin']:
...
def add_book():
    ...
    data = request.form
    file = request.files.get('book_pic')
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            save_path = os.path.join(app.config['BOOK_PICS_FOLDER'], filename)
            file.save(save_path)
            new_book.book_pic = filename  # Save only the filename
...
def update_book(id):
    ...
    book.year_published = int(data.get('year_published', book.year_published))
    book.loan_type = int(data.get('loan_type', book.loan_type))
    ...
    if file and allowed_file(file.filename):
        ...
        save_path = os.path.join(app.config['BOOK_PICS_FOLDER'], filename)
        file.save(save_path)
        book.book_pic = filename  # Save only the filename
